...
def runDTTA(Agents, Tasks, eps):
    ...
    # Remove global_max_j from remained except the winning agent_id
    for a in Agents:
        if a != agent_id:
            remained[a].remove(global_max_j)
            remained_theta[a].remove(global_max_j)
    ...
    while theta >= final_threshold:
        ...
        # omega_max = max([max(Omega[a]) for a in Agents and Omega[a]]) # Eliminate empty Omega[a] 
