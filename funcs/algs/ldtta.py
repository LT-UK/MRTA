...
Task allocation algorithm: 
    LDTTA: Lazy Decreasing Threshold Task Allocation
...
# 
# =============================================================================
def runLDTTA(Agents, Tasks, eps):
    '''
    Lazy Decreasing Threshold Task Allocation.
    Input:    
    ...
    while theta >= final_threshold:
        ...
        while 1 in flag_agent: 
            ...
            for a in Agents:
                ...
                if sorted_remained[a]:  # If sorted_remained[a] is not empty.
                    while sorted_mgvs[a][0] >= theta:
                        ...
                        if new_mgv >= theta:
                            ...
                            sorted_mgvs[a].pop(0) 
                            if not sorted_mgvs[a]:  # If sorted_mgvs[a] is empty.
                                break
                        else: # Re-sort sorted_mgvs and sorted_remained
                ...
                if not omega: # There is no task whose mgv is larger than the current threshold.
                    flag_agent[a] = 0
            if A: # len(A) > 0, new consensus round
                consensus_steps += 1 
