...
def MonteCarlo_ChangeNt():
    ...
    item_ctr = 0
    total_item = init.MC_ctr*((init.Nt_end-init.Nt_start)//init.Nt_step+1)
    # Print initial status.
    pb.printProgressBar(item_ctr, total_item, start_time, bar_color)
    
    # Change the number of agents       
    for Nt in range(init.Nt_start, init.Nt_end+1, init.Nt_step):
        # Na: The number of Agents
        init.Nts.append(Nt)
        
        Tasks = [j for j in range(Nt)]    
        init.Dist_mat = [[0 for j in range(Nt)] for i in range(Nt)]
...
#                    [round(y,3) for y in np.random.uniform(low=0.0, high=init.L, size=Na)]]       
    ...
    #            M = np.random.uniform(m_low,m_high,(Na,Nt))
                init.M = [[round(m,3) for m in 
                           np.random.uniform(init.m_low,init.m_high,Nt)] 
            ...
            # non-monotone
                ...
                m2 = [[round(m,3) for m in 
                       np.random.uniform(init.m_low,init.m_high,Nt-init.Na)] 
                        for a in range(init.Na)
                     ]
...
## STBTA                 
    ...
    if init.monotonicity:
        store_path = os.path.abspath("output") + "/montecarlo_nt/monotone"
    else:
        store_path = os.path.abspath("output") + "/montecarlo_nt/non_monotone"
...
# 
# =============================================================================
print("----- montecarlo_change_nt.py is loaded -----")
